...
export default function Guide({ exportGuide, apiKey }: { exportGuide: boolean, apiKey?: string }) {
    ...
    return(
       ...
       {etapes.map((etape, index) => (
                <Container key={index} className={index === 0 ? "ds-mt-25" : "ds-mt-10 ds-mb-25"}>
                    <b className="ds-text-primary">{etape.title} : </b>
                        ...
                        <div className="ds-flex ds-align-center ds-mt-13">
                            {etape.details.map((detail, i) => (
                                <div key={i}>
                                    ...
                                    />
                                    <Text text={detail.label} type={TextType.caption} className="ds-ml-13 ds-text-neutral500" />
                                </div>
                            ))}
                        </div>
                    ) : (
    ...
    <Container className="ds-mt-27 ds-mb-25"> 
      ...
      <CopyButton
      copy={copy}
      className="ds-flex ds-justify-end ds-bg-dark ds-text-white ds-mt-2"
      type="tertiary"
      handleCopy={handleCopy}
      />
      </div>
                <Card.Body>
                  <div className = "ds-text-white ds-text-size-12" ref={codeRef}>
                  <Documentation
                  user={authData?.id}
                  apiKey={apiKey}
                  />
